 ...
 *
 *
 * It is kept in double precision because the extra precision has sometimes shown much better accuracy
...
//!----------------------------------------------------------------------------
//!*******************************************************************
//! subroutine  initialize_loop_closure(bondLength, bondAngle, t_ang)
//!*******************************************************************
...
//! input:
//!
//  * bondLength(1:6): bond lengths (A1-C1, C1-N2, ..., N3-A3)
//!  * bondAngle(1:7): bond angles (N1-A1-C1, A1-C1-N2, ..., N3-A3-C3)
...
//  ! parameters for tripeptide loop (including bond lengths & angles)
  double[] len0 = new double[6], bondAngle0 = new double[7], t_ang0 = new double[2];
...
//  ! used for polynomial coefficients
  ...
  //These are molecule residue numbers, and there should be three of them, in ascending order
  //This was not part of the C version
  TripeptideClosure( Molecule m, int res[] ){

      double bondLength[] = new double[6];
      double bondAngle[] = new double[7];
      ...
      for(int a=0;a<3;a++){
          ...
          for(int b=0;b<3;b++){
              ...
              CACoord[a][b] = CACoordf[b];
              CCoord[a][b] = CCoordf[b];
          }
      }
      ...
      vec[0] = rm.subtract(CACoord[0], NCoord[0]);
      vec[1] = rm.subtract(CCoord[0], CACoord[0]);
      vec[2] = rm.subtract(NCoord[1], CCoord[0]);
      vec[3] = rm.subtract(CACoord[1], NCoord[1]);
      vec[4] = rm.subtract(CCoord[1], CACoord[1]);
      vec[5] = rm.subtract(NCoord[2], CCoord[1]);
      vec[6] = rm.subtract(CACoord[2], NCoord[2]);
      vec[7] = rm.subtract(CCoord[2], CACoord[2]);

      for(int a=0; a<6; a++)
          bondLength[a] = rm.norm(vec[a+1]);

      for(int a=0; a<7; a++)
          bondAngle[a] = rm.getAngle( rm.scale(vec[a], -1), vec[a+1]);

      t_ang[0] = calc_dih_ang( vec[1], vec[2], vec[3] );
      t_ang[1] = calc_dih_ang( vec[4], vec[5], vec[6] );

      initialize_loop_closure(bondLength, bondAngle, t_ang);
  }

  

  public void initialize_loop_closure(double bondLength[], double bondAngle[], double t_ang[])
 {
  double len1, len2, a_min, a_max;
  double[]  rr_c1 = new double[3], 
          rr_a2 = new double[3],  rr_c1a1 = new double[3];
...
//  do i = 0, 1
	for(i=0;i<2;i++)
	{
		...
		rr_a1[0] = Math.cos(bondAngle0[3*i+1])*len0[3*i];
		rr_a1[1] = Math.sin(bondAngle0[3*i+1])*len0[3*i];
		rr_a1[2] = 0;
		rr_n2[0] = len0[3*i+1];
		rr_n2[1] = 0;
		rr_n2[2] = 0;
		...
		rr_n2a2_ref[0] = -Math.cos(bondAngle0[3*i+2])*len0[3*i+2];
		rr_n2a2_ref[1] = Math.sin(bondAngle0[3*i+2])*len0[3*i+2];
		rr_n2a2_ref[2] = 0;

		rm.getRotMatrixRad(1, 0, 0, t_ang0[i], Us);

		mulpro = rm.applyRotMatrix(Us, rr_n2a2_ref);
		for(j=0;j<3;j++)
		{
		...
		xi[i+1] = rm.getAngle(tmp_val, bb_a2n2);
		...
		eta[i] = rm.getAngle(bb_a1a2, tmp_val);
  ...
  a_min = bondAngle[3] - (xi[1] + eta[1]);
  a_max = Math.min((bondAngle[3] + (xi[1] + eta[1])), pi);
...
/******************************************************************************
 * get_input_angles (Documentation for sanity)
 * input: cartesian coordinates of n1, a1, a3, and c3, also known as the 
 * "endpoints" of the loop closure problem, which are constraints.
 * Notation: a = C_alpha, n = N, C = backbone carbon!
 ******************************************************************************/
public void get_input_angles(double r_n1[], double r_a1[], double r_a3[], double r_c3[])
 {
  ...
  dr_sqr = rm.dot(r_a1a3,r_a1a3);
  //distance from a1 to a3
  len_aa[0] = Math.sqrt(dr_sqr);

	//Is the distance outisde of the feasible space reachable?
...
//  ! bond lengths
//  Length from a1 to n1
  for(i=0;i<3;i++)
    r_a1n1[i] = r_n1[i] - r_a1[i];

  len_na[0] = Math.sqrt(rm.dot(r_a1n1,r_a1n1));
  len_na[1] = len0[2];
  len_na[2] = len0[5];
  ...
  len_ac[0] = len0[0];
  len_ac[1] = len0[3];
  len_ac[2] = Math.sqrt(rm.dot(r_a3c3,r_a3c3));
...
//  ! delta(3): dih of N(1)CA(1)CA(3)C(3)
  ...
  for(i=0;i<3;i++)
    tmp_val[i] = -b_a1a3[i];
  xi[0] = rm.getAngle(tmp_val, b_a1n1);

  eta[2] = rm.getAngle(b_a1a3, b_a3c3);
...
//  ! theta (N, CA, C) bond angle
  theta[0] = bondAngle0[0];
  theta[1] = bondAngle0[3];
  theta[2] = bondAngle0[6];
...
//  ! check for existence of soln
  for(i=0;i<3;i++)
   {
    test_two_cone_existence_soln(theta[i], xi[i], eta[i], alpha[i]);
    if (n_soln == 0)
      return;
   }
...
public void test_two_cone_existence_soln(double tt, double kx, double et, double ap)
 {
...
void dihedrals_from_poly_roots(double roots[], double r_n1[], double r_a1[], double r_a3[], double r_c3[], double r_soln_n[][][], double r_soln_a[][][], double r_soln_c[][][])
{
	return;
}
...
void coord_from_poly_roots(double roots[], double r_n1[], double r_a1[], double r_a3[], double r_c3[], double r_soln_n[][][], double r_soln_a[][][], double r_soln_c[][][])
 {
  ...
  double[][] r_n = new double[3][3], r_a = new double[3][3], r_c = new double[3][3], Us = new double[3][3];
  double[] p = new double[4];
...
//  call cross(r_a1n1, ex, ez)
  ez = rm.cross(r_a1n1, ex);

  tmp_value = Math.sqrt(rm.dot(ez,ez));
  for(i=0;i<3;i++)
    ez[i] = ez[i]/tmp_value;
...
//  ! initial sig(1)
  ...
  angle = rm.getAngle(s1[0], r_tmp);
  sig1_init = Math.copySign(angle, rm.dot(r_tmp,s2[0]));
...
//     ! rotate back atoms by -(sig(1) - sig1_init) around -ex
     ...
     rm.getRotMatrixRad(-ex[0], -ex[1], -ex[2], tmp_value, Us);
     ...
     mat1 = rm.applyRotMatrix(Us, mat11);
     mat2 = rm.applyRotMatrix(Us, mat22);
     mat3 = rm.applyRotMatrix(Us, mat33);
     mat4 = rm.applyRotMatrix(Us, mat44);
     mat5 = rm.applyRotMatrix(Us, mat55);
	...
	for(i=0;i<3;i++)
	 {
        ...
        a1c1 = Math.sqrt(rm.dot(rr_a1c1, rr_a1c1));
        c1n2 = Math.sqrt(rm.dot(rr_c1n2, rr_c1n2));
        n2a2 = Math.sqrt(rm.dot(rr_n2a2, rr_n2a2));
        a2c2 = Math.sqrt(rm.dot(rr_a2c2, rr_a2c2));
        c2n3 = Math.sqrt(rm.dot(rr_c2n3, rr_c2n3));
        n3a3 = Math.sqrt(rm.dot(rr_n3a3, rr_n3a3));
        a1a2 = Math.sqrt(rm.dot(rr_a1a2, rr_a1a2));
        a2a3 = Math.sqrt(rm.dot(rr_a2a3, rr_a2a3));
	...
	for(i=0;i<3;i++)
        ...
        for(i=0;i<3;i++)
	 {
	  ...
	  tmp_array2[i] = -rr_n2a2[i]/n2a2;
        ...
        n2a2c2 = rm.getAngle(tmp_array1, tmp_array2);

        System.out.printf("ang_nac = %9.3f%9.3f\n", bondAngle0[0]*rad2deg, n1a1c1*rad2deg);
        System.out.printf("ang_nac = %9.3f%9.3f\n", bondAngle0[3]*rad2deg, n2a2c2*rad2deg);
        System.out.printf("ang_nac = %9.3f%9.3f\n", bondAngle0[6]*rad2deg, n3a3c3*rad2deg);
...
//!-----------------------------------------------------------------------
  ...
  arg = rm.dot(p,q)/Math.sqrt(rm.dot(p,p)*rm.dot(q,q));

  arg = Math.copySign(Math.min(Math.abs(arg),1),arg);
  return Math.copySign(Math.acos(arg), rm.dot(s,r2));
 }
