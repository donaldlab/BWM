...
public class KSParser
{
	...
	public void handleKSMaster(String s) {
		...
		RotamerSearch rs = new RotamerSearch(m,strandMut,numberMutable, strandsPresent,hElect,hVDW,hSteric,true,true,
		...
		for(int i=0; i<m.numberOfStrands;i++){
			if ((new Boolean((String)sParams.getValue("USEUNBOUNDSTRUCT"+i, "false"))).booleanValue()){ //a different input structure is used for the unbound partition function computation
				...
				rs = new RotamerSearch(m,strandMut,numberMutable, strandsPresent,hElect,hVDW,hSteric,true,true,
						epsilon,stericThresh,softStericThresh,distDepDielect,dielectConst,doDihedE,doSolvationE,solvScale,
						softvdwMultiplier,grl, doPerturbations, pertFile, minimizePerts, false, false);
	...
	private Set<OneMutation> handleHybridKSLoadMutList (String mutFileName, int numMutable,
		...
		if (mutSet == null) {
			...
			// Create the mutation list with estimated energies
			mutSet = new TreeSet<OneMutation>();
			RotamerSearch rs = new RotamerSearch(m, strandMut, numMutable, strandsPresent, hElect, hVDW, hSteric, true,
					true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst,doDihedE,
	...
	private CommucObj hybridKScompute(CommucObj cObj){
		...
		for(int runNum = 0; runNum<allCombos.length; runNum++) {
			...
			RotamerSearch rs = new RotamerSearch(m,strandMut,numberMutable,strandsPresent, hElect, hVDW, hSteric, true,
	...
	public CommucObj handleComputeAllPairwiseRotamerEnergiesSlave(CommucObj cObj) {
		...
		RotamerSearch rs = new RotamerSearch(m,cObj.strandMut, cObj.mutableSpots, cObj.strandsPresent, hElect, hVDW, hSteric, true,
	...
	public void computeEnergyOfConformations(String s)
	{
		...
		RotamerSearch rs = new RotamerSearch(mp.m,mp.strandMut,mp.numberMutable, mp.strandsPresent, hElect, hVDW, hSteric, true,
		...
		InteractionGraph sparseGraph = InteractionGraph.loadFromFile(runName);
		rs.getMinMatrix().G = sparseGraph;
		...
		for (int curResult=0; curResult<numResults; curResult++){
			...
			System.out.print("Analyzing result "+(curResult+1)+"..");
			...
			int[] curAAs = new int[strandMut[0].length];
			int[] pdbNumbers = new int[strandMut[0].length];
			int ctr = 0;
			for(int str=0;str<numOfStrands;str++){	
				for(int j=0;j<strandMut[str].length;j++){
					...
					curAAs[j] = curSeqInd[ctr];
					pdbNumbers[j] = mp.m.mapMolResNumToPDBResNum(strandMut[str][j]);
					strandRot[str].setAllowable(strandMut[str][j],curSeq[ctr]);
					rs.strandRot[str].setAllowable(strandMut[str][j],curSeq[ctr]);
			...
			ctr = 0;
			rs.printOneAndTwoBodyEnergies(pdbNumbers, curAAs, curRot);
	...
	public void handleDoDEE(String s) {
		...
		do{
			...
			if(genInteractionGraph)
			{
				...
				try 
				{
					loadedPruneObject = PrunedRotamers.createFromFiles(sParams.getValue("RUNNAME"));
					System.out.println("Checking for previous pruned object info...");
			...
			if (!doDACS){ //DACS will not be performed
				...
				if (genInteractionGraph && !doSparseAStar) //SJ - generate interaction graph and quit if doSparseAStar is false, also added the return of error
					sparseGraphError=genInteractionGraph(mp.numberMutable, rs, prunedRotAtRes, runName, mp.strandMut, eInteractionCutoff, distCutoff, mp.m, preprocPairs, pairSt,mp.mutRes2Strand,mp.mutRes2StrandMutIndex , false, false);
					...
					if(doSparseAStar){ // SJ - generate interaction graph and also change energy matrix if doSparseAStar is true, and continue with enumeration
						sparseGraphError=genInteractionGraph(mp.numberMutable, rs, prunedRotAtRes, runName, mp.strandMut, eInteractionCutoff, distCutoff, mp.m, preprocPairs, pairSt,mp.mutRes2Strand,mp.mutRes2StrandMutIndex,true, false);
	...
	public void generateBackbones(String s){
		...
		RotamerSearch rs = new RotamerSearch(mp.m,mp.strandMut,numberMutable,strandsPresent, hElect, hVDW, hSteric, true,
				true, 0.0f, stericThresh, softStericThresh, distDepDielect, dielectConst, doDihedE, 
	...
	////////////////////////////////////////////////////////////////
	/**
	 * Setup and run loop closure
	 */
	public void closeLoop(String s, ParamSet sParams)
	{
		if (sParams==null){ //parameter files not read yet, so read them in
			sParams = new ParamSet();
			sParams.addParamsFromFile(getToken(s,2)); //read system parameters
		...
		Residue stuff = ppr.getResidue("ALA");
		Residue stuff2 = ppr.getResidue("GLY");
		//mp.m.addResidue(0, stuff);
		//mp.m.addResidue(0, stuff2);
		saveMolecule(mp.m, "loaded.pdb", 0);
		boolean debugRun = true;
		if(debugRun)
		{
			closeNAnchor(mp, 1, 14);
		}

		else
			for(int triple = 0; triple < mp.m.residue.length-2; triple++)
			{
				for(int anchor = 0; anchor < mp.m.residue.length; anchor++)
				{
					closeCAnchor(mp, triple, anchor);
					closeNAnchor(mp, triple, anchor);
				}
			}
	}

	private void closeNAnchor(MolParameters mp, int triple, int NAnchor)
	{
		closeAnchor(mp, triple, NAnchor, -1);
	}

	private void closeCAnchor(MolParameters mp, int triple, int CAnchor)
	{
		closeAnchor(mp, triple, -1, CAnchor);
	}

	private void closeAnchor(MolParameters mp, int triple, int NAnchor, int CAnchor)
	{
		System.out.println("ATtempting "+triple+", "+(triple+1)+", "+(triple+2)+", to N anchor "+NAnchor+" or C anchor "+CAnchor);

		int[] NAnchorAtoms = null;
		if(NAnchor >= 0)
		{
			NAnchorAtoms = new int[3];
			NAnchorAtoms[0] = mp.m.residue[NAnchor].getAtomNameToMolnum("N");
			NAnchorAtoms[1] = mp.m.residue[NAnchor].getAtomNameToMolnum("CA");
			NAnchorAtoms[2] = mp.m.residue[NAnchor].getAtomNameToMolnum("C");
		}

		int[] CAnchorAtoms = null;
		if(CAnchor >= 0)
		{
			CAnchorAtoms = new int[3];
			CAnchorAtoms[0] = mp.m.residue[CAnchor].getAtomNameToMolnum("N");
			CAnchorAtoms[1] = mp.m.residue[CAnchor].getAtomNameToMolnum("CA");
			CAnchorAtoms[2] = mp.m.residue[CAnchor].getAtomNameToMolnum("C");
		}	

		CloseLoop lca = new CloseLoop("", mp.m, new int[]{triple,triple+1,triple+2}, NAnchorAtoms, CAnchorAtoms);
		lca.debugWriter = this;

		if(lca.CalculateSolutions()>0)
		{
			int anchor = Math.max(CAnchor, NAnchor);
			System.out.println("SOLUTION FOR "+triple+", to "+anchor);
			for(int soln = 0; soln < lca.solutions; soln++)
			{

				lca.applySolution(soln);
				saveMolecule(mp.m, "output-"+triple+"-"+anchor+"-"+soln+".pdb", 0);
				System.out.println("Saved solution "+soln+" as "+"output-"+triple+"-"+anchor+"-"+soln+".pdb");
				//lca.restore();
				saveMolecule(mp.m, "output-"+triple+"-"+anchor+"-"+soln+"-restored.pdb", 0);
	...
	private CommucObj handleDoResEntropySlave(CommucObj cObj){
		...
		if (cObj.compASdist){ //AS-AS distance computation
			...
			RotamerSearch rs = new RotamerSearch(m, cObj.strandMut, cObj.mutableSpots, strandsPresent, hElect, hVDW, hSteric, true,
					true, cObj.epsilon, cObj.stericThresh, cObj.softStericThresh, cObj.distDepDielect, 
					cObj.dielectConst, cObj.doDihedE,cObj.doSolvationE,cObj.solvScale,cObj.vdwMult, grl,
					false, null, false, false, false);
